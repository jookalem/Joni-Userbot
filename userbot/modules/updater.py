...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if build.status == "failed":
            ...
            await edit_delete(event,
                              "`Joni-Userbot Berhasil DiUpdate😉,Restart Tunggu Sebentar`"
                              )

        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, "#BOT \n" "`Joni-Userbot Berhasil Di Update`"
            )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    x = await edit_or_reply(event, "**✨ᴊᴏɴɪ-υѕᴇʀвσт✨** `Berhasil Di Update!`")
    await asyncio.sleep(1)
    await x.edit("**✨ᴊᴏɴɪ-υѕᴇʀвσт✨** `Di Restart....`")
    await asyncio.sleep(1)
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#BOT \n" "**✨ᴊᴏɴɪ-υѕᴇʀвσт✨ Telah Di Perbarui.**"
        )
...
async def upstream(event):
    ...
    if changelog == "" and force_update is False:
        await xx.edit(
            f"\n✨ᴊᴏɴɪ-υѕᴇʀвσт✨ Sudah Versi Terbaru || Tunggu Update Terbaru\n"
        )
    ...
    if conf is None and force_update is False:
        changelog_str = (
            f"**Pembaruan Untuk ✨ᴊᴏɴɪ-υѕᴇʀвσт✨ :\n\n⚒️ Pembaruan Data :**\n`{changelog}`"
        )
    ...
    if force_update:
        ...
        await xx.edit("` Proses Update ✨ᴊᴏɴɪ-υѕᴇʀвσт✨, Loading....1%`")
        await xx.edit("` Proses Update ✨ᴊᴏɴɪ-υѕᴇʀвσт✨ Loading....20%`")
        await xx.edit("` Proses Update ✨ᴊᴏɴɪ-υѕᴇʀвσт✨, Loading....35%`")
        await xx.edit("` Proses Update ✨ᴊᴏɴɪ-υѕᴇʀвσт✨, Loading....77%`")
        await xx.edit("` Proses Update ✨ᴊᴏɴɪ-υѕᴇʀвσт✨, Updating...90%`")
        await xx.edit(
            "` Proses Update ✨ᴊᴏɴɪ-υѕᴇʀвσт✨, Mohon Tunggu Sebentar....100%`"
        )
